...
The search script lives inside this skill's own directory. This project uses Windows PowerShell and the Python runtime already bundled with Codex. Initialize these variables in each new shell invocation (tool calls may not share shell state), then run the examples below:

```powershell
$uiUxPython = 'C:/Users/Zaladdin/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/python.exe'
$uiUxSearch = 'E:/E-repetitor/.agents/skills/ui-ux-pro-max/scripts/search.py'
```

Always use the resolved full paths; do not use Claude-specific environment variables. `-B` prevents Python from creating bytecode cache files.

```powershell
& $uiUxPython -B $uiUxSearch "<query>" --domain <domain>
```

Requires Python 3.x with the standard library only. If the bundled path changes, resolve the current Python executable through Codex workspace dependencies; do not install packages or another runtime implicitly.
...
```powershell
& $uiUxPython -B $uiUxSearch "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```powershell
& $uiUxPython -B $uiUxSearch "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```powershell
& $uiUxPython -B $uiUxSearch "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```powershell
& $uiUxPython -B $uiUxSearch "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```powershell
& $uiUxPython -B $uiUxSearch "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```powershell
& $uiUxPython -B $uiUxSearch "<keyword>" --domain <domain> [-n <max_results>]
```
...
```powershell
& $uiUxPython -B $uiUxSearch "<keyword>" --stack <stack>
```
...
& $uiUxPython -B $uiUxSearch "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
& $uiUxPython -B $uiUxSearch "keyboard focus modal" --domain ux

# Step 4: stack guidelines
& $uiUxPython -B $uiUxSearch "suspense streaming bundle" --stack nextjs
```
