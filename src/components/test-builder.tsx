...
export function TestBuilder({ accountId, testId, onSessionChanged, onChanged, onClose }: {
  ...
  if (!resource.data) return <Modal title="Конструктор теста" onClose={onClose}>{resource.loading ? <p role="status">Загружаем тест…</p> : <div className="form-error"><p role="alert">{resource.error}</p><button className="button secondary" onClick={resource.reload}>Повторить</button></div>}</Modal>;
...
function DraftEditor({ accountId, initial, onSessionChanged, onSaved, onPublished, onClose, onReload }: {
  ...
  async function save(publish = false) {
    ...
    }, publish ? 'Версия опубликована.' : 'Черновик сохранён.');
  ...
  return <Modal title={archived ? 'Архивный тест' : 'Конструктор теста'} onClose={close}>
    <div className="test-builder"><p className="muted">{initial.subjectName} · {draft.questions.length} вопросов · максимум {maxPoints} балл.</p>
      <p className="account-list-help muted">Изменения сохраняются в черновике. Каждая публикация создаёт отдельную версию; уже выданные тесты сохраняют прежние вопросы.</p>
      <div className="test-toolbar"><button type="button" className="button secondary small" onClick={() => setPreview(value => !value)} disabled={actions.busy}><Eye size={16} aria-hidden="true" />{preview ? 'Вернуться в конструктор' : 'Предпросмотр'}</button><span className="muted" role="status">{dirty ? 'Есть несохранённые изменения' : 'Черновик сохранён'}</span></div>
      {preview ? <TestPreview draft={draft} /> : <form className="account-form test-builder-form" onSubmit={submit}>
        <fieldset disabled={actions.busy || archived} className="test-form-fieldset">
          <label htmlFor={`${id}-title`}>Название</label><input id={`${id}-title`} value={draft.title} maxLength={200} required onChange={event => setDraft({ ...draft, title: event.target.value })} />
          <label htmlFor={`${id}-topic`}>Тема · необязательно</label><input id={`${id}-topic`} value={draft.topic ?? ''} maxLength={200} onChange={event => setDraft({ ...draft, topic: event.target.value })} />
          <label htmlFor={`${id}-instruction`}>Инструкция ученику · необязательно</label><textarea id={`${id}-instruction`} rows={3} maxLength={4000} value={draft.instruction ?? ''} onChange={event => setDraft({ ...draft, instruction: event.target.value })} />
          <label htmlFor={`${id}-pass`}>Проходной балл · необязательно</label><input id={`${id}-pass`} className="test-number-input" type="number" min={0} max={3000} step={0.01} value={passInput} onChange={event => { setPassInput(event.target.value); setDraft({ ...draft, passPoints: event.target.value === '' ? undefined : Number(event.target.value) }); }} />
          ...
          {!draft.questions.length && <p className="account-next-note">Начните с первого вопроса. Для публикации понадобится хотя бы один заполненный вопрос.</p>}
          <div className="test-add-questions">{(['single_choice', 'multiple_choice', 'text'] as const).map(type => <button key={type} type="button" className="button secondary small" disabled={draft.questions.length >= 30} onClick={() => addQuestion(type)}><Plus size={15} aria-hidden="true" />{type === 'single_choice' ? 'Один вариант' : type === 'multiple_choice' ? 'Несколько вариантов' : 'Текстовый ответ'}</button>)}</div>
          <div className="form-actions"><button type="submit" className="button secondary" disabled={!dirty}><Save size={17} aria-hidden="true" />Сохранить черновик</button><button type="button" className="button" disabled={!draft.questions.length || (!dirty && baseline.status === 'published')} onClick={() => setConfirmation('publish')}>{!dirty && baseline.status === 'published' ? 'Эта версия уже опубликована' : 'Опубликовать версию'}</button></div>
        </fieldset>
      </form>}
      <ActionFeedback actions={actions} />
      {actions.error && <button className="text-button" disabled={actions.busy} onClick={() => setConfirmation('reload')}>Загрузить актуальный черновик</button>}
      {confirmation && <div className="test-confirmation" role="group" aria-label="Подтверждение действия" tabIndex={-1} ref={confirmationRef}><p>{confirmation === 'publish' ? 'Сохранить изменения и опубликовать неизменяемую версию? После этого её можно назначить ученикам.' : confirmation === 'close' ? 'Закрыть конструктор и потерять несохранённые изменения?' : 'Загрузить черновик с сервера? Несохранённые изменения будут потеряны.'}</p><div className="form-actions"><button className="button secondary" disabled={actions.busy} onClick={() => setConfirmation(null)}>Отмена</button><button className="button" disabled={actions.busy} onClick={() => { if (confirmation === 'publish') void save(true); else if (confirmation === 'close') onClose(); else onReload(); }}>{confirmation === 'publish' ? 'Опубликовать' : confirmation === 'close' ? 'Закрыть без сохранения' : 'Загрузить'}</button></div></div>}
    </div>
  </Modal>;
}
