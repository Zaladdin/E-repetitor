...
export function Workspace() {
  ...
  if (!state) return <main className="loading-screen" aria-busy="true"><span className="loading-brand">E-repetitor</span><p>{t("Загружаем рабочее пространство…")}</p></main>;
  ...
  const nav = isTeacher ? [
    { id: 'today', label: t("Сегодня"), Icon: CalendarDays },
    { id: 'students', label: t("Ученики"), Icon: UsersRound },
    { id: 'subjects', label: t("Предметы"), Icon: BookOpen },
    { id: 'requests', label: t("Запросы"), Icon: MessageSquare },
  ] : [
    { id: 'today', label: isParent ? t("Мои дети") : t("Мои предметы"), Icon: isParent ? UsersRound : BookOpen },
    { id: 'requests', label: t("Запросы"), Icon: MessageSquare },
  ];
  const title = nav.find((item) => item.id === page)?.label ?? t("Сегодня");
  const subtitle = isTeacher && page === 'today' ? t("Вторник, 22 сентября")
    : page === 'requests' ? actor.role === 'student' ? t("Вы решаете, кому открыть доступ") : t("Статусы ваших подключений")
      : isTeacher ? page === 'students' ? t("Обучение в вашем рабочем пространстве") : t("Ваши предметы и направления")
        : isParent ? t("Обучение ребёнка в одном кабинете") : t("Все преподаватели в одном месте");
  ...
  function command(value: Command) {
    ...
    if (value.type === 'request_enrollment' || value.type === 'request_parent_connection') {
      setNotice(t("Запрос отправлен. Переключитесь на профиль ученика, чтобы подтвердить его."));
    } else if (value.type === 'create_subject') setNotice(t("Предмет создан. Теперь можно подключить к нему ученика."));
  }

  return <div className="app-shell">
    <a className="skip-link" href="#main-content">{t("Перейти к содержимому")}</a>
    <div className="mobile-header"><strong>E-repetitor</strong><button className="icon-button" aria-label={mobileMenu ? t("Закрыть меню") : t("Открыть меню")}
      aria-expanded={mobileMenu} aria-controls="sidebar-nav" onClick={() => setMobileMenu(!mobileMenu)}>{mobileMenu ? <X /> : <Menu />}</button></div>
    <aside className={`sidebar ${mobileMenu ? 'is-open' : ''}`}><div className="brand">E-repetitor</div><p className="brand-caption">{t("Рабочее пространство")}</p>
      <nav id="sidebar-nav" aria-label={t("Главная навигация")}>{nav.map(({ id, label, Icon }) => <button key={id}
      ...
      <div className="sidebar-user"><span className="avatar">{profile.name.split(' ').map((part) => part[0]).slice(0, 2).join('')}</span>
        <div><strong>{profile.name}</strong><span>{t(roleLabels[actor.role])}</span></div></div>
    </aside>
    <div className="workspace-body">
      <div className="demo-bar"><span>{t("Демо · данные хранятся в этом браузере")}</span><LanguageSwitcher /><button onClick={() => setModal('reset')}>{t("Сбросить демо")}</button></div>
      <div className="account-bar"><label className="sr-only" htmlFor="demo-account">{t("Демо-профиль")}</label>
        <select id="demo-account" value={`${actor.userId}:${actor.role}`} onChange={(event) => {
          ...
          {item.label.split(' ')[0]} · {t(roleLabels[item.role]).toLocaleLowerCase(localeTag())}
        </option>)}</select></div>
      <main ref={mainRef} id="main-content" className="main-content" tabIndex={-1}>
        <header className="page-heading"><div><h1>{title}</h1><p>{subtitle}</p></div>
          {(isTeacher || isParent) && <button className="button primary-action" onClick={() => setModal(isTeacher && page === 'subjects' ? 'subject' : 'connection')}>
            <Plus size={22} aria-hidden="true" />{isTeacher ? page === 'subjects' ? t("Новый предмет") : t("Подключить ученика") : t("Добавить ребёнка")}</button>}
        </header>
        {state.storageWarning && <p className="storage-warning" role="alert">{t(state.storageWarning)}</p>}
    ...
    {modal === 'reset' && <ConfirmDialog title={t("Начать демонстрацию заново?")} description={t("Созданные в демо предметы и подключения будут заменены исходными примерами. Это действие затронет только этот браузер.")}
      action={t("Сбросить демо")} onClose={() => setModal(null)} onConfirm={() => { reset(); setModal(null); setNotice(t("Демонстрация сброшена.")); }} />}
  </div>;
}
