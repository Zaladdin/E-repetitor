...
export function CreatePaymentRecord({ accountId, onSessionChanged, onClose, onCreated }: FormProps & { onCreated: () => void }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    await actions.run(() => accountApi.createPaymentRecord(accountId, { ...input, requestId: retry.current!.key }), t("Запись добавлена."));
  }
  function close() { if (!actions.busy) { if (dirty) setClosing(true); else onClose(); } }

  return <Modal title={t("Добавить запись об оплате")} onClose={close}>
    <p className="muted">{t("Создайте запись за занятие, пакет или период. Получив оплату, отметьте её галочкой в журнале.")}</p>
    <form className="account-form account-payment-form" onSubmit={create} onChange={() => setDirty(true)} aria-busy={actions.busy}>
      <label htmlFor={`${id}-enrollment`}>{t("Ученик и предмет")}</label><select id={`${id}-enrollment`} name="enrollmentId" required defaultValue="" disabled={blocked}>
      ...
      <label htmlFor={`${id}-title`}>{t("За что оплата")}</label><input id={`${id}-title`} name="title" required maxLength={200} placeholder={t("Например, математика за сентябрь")} disabled={blocked} aria-describedby={`${id}-shared`} />
      <p id={`${id}-shared`} className="account-list-help muted">{t("Название, сумма и статус видны ученику и родителю с подтверждённым доступом.")}</p>
      <div className="account-payment-money-fields"><div><label htmlFor={`${id}-amount`}>{t("Сумма · необязательно")}</label><input id={`${id}-amount`} inputMode="decimal" value={amount} onChange={event => setAmount(event.target.value)} maxLength={16} placeholder={locale === 'en' ? '120.00' : '120,00'} disabled={blocked} aria-describedby={`${id}-amount-help`} /></div>
        <div><label htmlFor={`${id}-currency`}>{t("Валюта")}</label><select id={`${id}-currency`} name="currency" defaultValue="AZN" disabled={blocked || !amount.trim()}>{PAYMENT_CURRENCIES.map(currency => <option key={currency}>{currency}</option>)}</select></div>
      </div>
      <p id={`${id}-amount-help`} className="account-list-help muted">{t("Если нужен только статус, оставьте сумму пустой. Сумма относится ко всей записи.")}</p>
      {inputError && <p ref={errorRef} tabIndex={-1} role="alert" className="form-error">{t(inputError)}</p>}
      <ActionFeedback actions={actions} />
      <div className="form-actions"><button type="button" className="button secondary" disabled={actions.busy} onClick={close}>{t("Закрыть")}</button><button className="button" disabled={blocked || closing}>{actions.busy ? t("Сохраняем…") : t("Добавить как неоплаченную")}</button></div>
    </form>
    {closing && <div className="account-test-confirm" role="group" aria-label={t("Закрытие записи")} ref={closingRef} tabIndex={-1}><p>{t("Закрыть форму без сохранения?")}</p><div className="form-actions"><button className="button secondary" onClick={() => setClosing(false)}>{t("Продолжить заполнение")}</button><button className="button" onClick={onClose}>{t("Закрыть без сохранения")}</button></div></div>}
  </Modal>;
}
...
export function PaymentCorrection({ accountId, record, action, onSessionChanged, onClose, onChanged }: FormProps & {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    await actions.run(() => action === 'unpay'
      ...
      : accountApi.cancelPaymentRecord(accountId, record.id, { version: record.version, reason }), action === 'unpay' ? t("Отметка снята.") : t("Запись отменена."));
  }
  return <Modal title={action === 'unpay' ? t("Снять отметку «Оплачено»") : t("Отменить запись")} onClose={() => { if (!actions.busy) onClose(); }}>
    ...
    <p className="muted">{action === 'unpay' ? t("Запись снова будет показана как неоплаченная. Предыдущая отметка останется в истории.") : t("Ошибочная запись останется в истории со статусом «Отменена». Восстановить её нельзя; при необходимости создайте новую.")}</p>
    <form className="account-form account-payment-form" onSubmit={submit} aria-busy={actions.busy}>
      <label htmlFor={`${id}-reason`}>{t("Причина исправления")}</label><textarea id={`${id}-reason`} name="reason" required minLength={3} maxLength={1000} rows={3} disabled={actions.busy} aria-describedby={`${id}-reason-help`} />
      <p id={`${id}-reason-help`} className="account-list-help muted">{t("Причину видите только вы в истории записи.")}</p>
      <ActionFeedback actions={actions} />
      <div className="form-actions"><button type="button" className="button secondary" disabled={actions.busy} onClick={onClose}>{t("Закрыть")}</button><button className="button" disabled={actions.busy}>{actions.busy ? t("Сохраняем…") : action === 'unpay' ? t("Снять отметку") : t("Отменить запись")}</button></div>
    </form>
  </Modal>;
}
...
export function PaymentHistory({ accountId, record, onSessionChanged, onClose }: FormProps & { record: PaymentRecord }) {
  ...
  return <Modal title={t("История оплаты")} onClose={onClose}>
    <p className="account-payment-dialog-summary">{record.studentName} · {record.subjectName}<br /><strong>{record.title}</strong><br />{formatPaymentAmount(record)}</p>
    <PageContent page={history} emptyTitle={t("История пока пуста")} emptyText={t("Изменения статуса будут отображаться здесь.")} disabled={false}>
      <ol className="account-payment-history">{history.items.map(event => <li key={event.id}><h3>{t(HISTORY_LABELS[event.type])}</h3><p><time dateTime={event.occurredAt}>{dateTime(event.occurredAt)}</time> · {event.actorName}</p>
        {event.previousPaidMarkedAt && <p>{t("Предыдущая отметка оплаты:")}{' '}{dateTime(event.previousPaidMarkedAt)}</p>}{event.reason && <p className="account-payment-reason">{t("Причина:")}{' '}{event.reason}</p>}
      </li>)}</ol>
    </PageContent>
