...
function ReviewForm({ accountId, attempt, onSessionChanged, onClose, onSaved, onReload }: {
  ...
  return <Modal title={t("Ответы и проверка")} onClose={close}><div className="test-review">
    ...
    {attempt.score !== undefined && <p className="test-score">{attempt.status === 'waiting_review' ? t("Автоматическая часть: ") : ''}{attempt.score} / {attempt.maxPoints} {t(" балл.")}{attempt.status !== 'waiting_review' && attempt.percentage !== undefined ? ` · ${attempt.percentage}%` : ''}</p>}
      ...
      <fieldset className="test-form-fieldset" disabled={actions.busy || !canReview}>
        {questions.map((question, index) => {
          ...
          return <fieldset key={question.id} className="test-question-editor"><legend>{t("Вопрос ")}{index + 1} {t(" · максимум ")}{question.points} {t(" балл.")}</legend><p className="test-preserve-text">{question.prompt}</p>
            ...
            {question.type === 'text' && <><label htmlFor={`${id}-${question.id}-points`}>{t("Балл за ответ")}</label><input id={`${id}-${question.id}-points`} name={`points-${question.id}`} type="number" min={0} max={question.points} step={0.01} required defaultValue={grade?.points ?? ''} className="test-number-input" /><label htmlFor={`${id}-${question.id}-comment`}>{t("Комментарий ученику после публикации · необязательно")}</label><textarea id={`${id}-${question.id}-comment`} name={`comment-${question.id}`} rows={2} maxLength={1000} defaultValue={grade?.comment ?? ''} /></>}
            {question.type !== 'text' && grade && <p className="account-list-help">{t("Автоматическая оценка: ")}{grade.points} / {question.points}</p>}
          </fieldset>;
        })}
        <label htmlFor={`${id}-comment`}>{t("Общий комментарий ученику и родителю после публикации")}</label><textarea id={`${id}-comment`} name="comment" rows={3} maxLength={2000} defaultValue={attempt.comment ?? ''} />
    ...
    {attempt.status === 'completed' && <><p className="account-list-help muted">{t("Проверенный результат пока виден только вам. Публикация откроет баллы и комментарии ученику и связанным родителям.")}</p><button className="button" disabled={actions.busy || dirty} onClick={() => setPublishConfirmation(true)}>{t("Опубликовать результат")}</button>{dirty && <p className="muted account-list-help">{t("Сначала сохраните изменённые оценки и комментарии.")}</p>}</>}
