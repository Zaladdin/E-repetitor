...
function findStudent(state: DemoState, publicId: string): StudentProfile {
  if (!validateStudentId(publicId)) throw new DomainError("Введите Student ID в формате STU-K7M4-P92X.", "INVALID_STUDENT_ID");
  ...
  if (!student || !state.users.some((user) => user.id === student.userId && user.status === "active")) {
    throw new DomainError("Не удалось отправить запрос. Проверьте Student ID у ученика.", "STUDENT_UNAVAILABLE");
  }
  return student;
}
...
function createSubject(state: DemoState, actor: Actor, command: Extract<Command, { type: "create_subject" }>, context: CommandContext): DemoState {
  ...
  if (!name || name.length > 100) throw new DomainError("Название предмета должно содержать от 1 до 100 символов.", "INVALID_SUBJECT_NAME");
  if (state.subjects.some((item) => item.teacherId === teacher.id && item.status === "active" && item.name.toLocaleLowerCase("ru") === name.toLocaleLowerCase("ru"))) {
    throw new DomainError("Такой предмет уже добавлен в ваш кабинет.", "DUPLICATE_SUBJECT");
  }
...
function requestEnrollment(state: DemoState, actor: Actor, command: Extract<Command, { type: "request_enrollment" }>, context: CommandContext): DemoState {
  ...
  if (!subject) throw new DomainError("Выберите активный предмет из своего кабинета.", "SUBJECT_UNAVAILABLE");
  ...
  if (existing) throw new DomainError("Ученик уже подключён к этому предмету. Откройте существующую запись.", "DUPLICATE_ENROLLMENT");
...
function decideEnrollment(state: DemoState, actor: Actor, command: Extract<Command, { type: "decide_enrollment" }>, context: CommandContext): DemoState {
  ...
  if (!enrollment) throw new DomainError("Запрос недоступен.", "NOT_FOUND");
  if (enrollment.status !== "pending") throw new DomainError("Этот запрос уже обработан или срок его действия истёк.", "INVALID_TRANSITION");
  if (command.decision !== "accept" && command.decision !== "reject") throw new DomainError("Выберите подтверждение или отклонение запроса.", "INVALID_DECISION");
...
function setEnrollmentStatus(state: DemoState, actor: Actor, command: Extract<Command, { type: "set_enrollment_status" }>, context: CommandContext): DemoState {
  ...
  if (!enrollment) throw new DomainError("Запись ученика недоступна.", "NOT_FOUND");
  const allowed = enrollment.status === "active" ? ["paused", "completed", "cancelled"] : enrollment.status === "paused" ? ["active", "completed", "cancelled"] : [];
  if (!allowed.includes(command.status)) throw new DomainError("Этот переход статуса недоступен. Подключение подтверждает ученик.", "INVALID_TRANSITION");
...
function requestParentConnection(state: DemoState, actor: Actor, command: Extract<Command, { type: "request_parent_connection" }>, context: CommandContext): DemoState {
  ...
  const student = findStudent(state, command.publicId);
  if (student.userId === actor.userId) throw new DomainError("Нельзя добавить собственный профиль в качестве ребёнка.", "SELF_CONNECTION");
  ...
  if (existing?.status === "pending") return state;
  if (existing) throw new DomainError("Ребёнок уже добавлен в ваш кабинет.", "DUPLICATE_PARENT_CONNECTION");
...
function decideParentConnection(state: DemoState, actor: Actor, command: Extract<Command, { type: "decide_parent_connection" }>, context: CommandContext): DemoState {
  ...
  if (!connection) throw new DomainError("Запрос недоступен.", "NOT_FOUND");
  if (connection.status !== "pending") throw new DomainError("Этот запрос уже обработан.", "INVALID_TRANSITION");
  if (command.decision !== "accept" && command.decision !== "reject") throw new DomainError("Выберите подтверждение или отклонение запроса.", "INVALID_DECISION");
  const status = command.decision === "accept" ? "active" : "rejected";
...
function revokeParentConnection(state: DemoState, actor: Actor, command: Extract<Command, { type: "revoke_parent_connection" }>, context: CommandContext): DemoState {
  ...
  if (!connection) throw new DomainError("Связь недоступна.", "NOT_FOUND");
  if (connection.status !== "active") throw new DomainError("Отозвать можно только подтверждённую связь.", "INVALID_TRANSITION");
...
export function executeCommand(state: DemoState, actor: Actor, command: Command, context = defaultContext()): DemoState {
  ...
  switch (command.type) {
    ...
    default: throw new DomainError("Неизвестное действие.", "UNKNOWN_COMMAND");
  }
}
