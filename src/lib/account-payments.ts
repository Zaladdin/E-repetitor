...
export function parsePaymentAmount(input: string): number | undefined {
  ...
  if (!/^\d{1,7}(?:[.,]\d{1,2})?$/.test(value)) throw new Error(translate("Укажите сумму от 0,01 до 9 999 999,99, не более двух знаков после запятой."));
  const [whole, fraction = ''] = value.replace(',', '.').split('.');
  const minor = Number(whole) * 100 + Number(fraction.padEnd(2, '0'));
  if (minor < 1 || minor > 999999999) throw new Error(translate("Сумма должна быть от 0,01 до 9 999 999,99."));
  return minor;
}

export function formatPaymentAmount(record: Pick<PaymentRecord, 'amountMinor' | 'currency'>): string {
  if (record.amountMinor === undefined || record.currency === undefined) return translate("Сумма не указана");
  return new Intl.NumberFormat(localeTag(), { style: 'currency', currency: record.currency, currencyDisplay: 'code', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(record.amountMinor / 100);
}
